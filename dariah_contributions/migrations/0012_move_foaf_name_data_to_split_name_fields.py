...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for item in orm.DcCreator.objects.all():
            item.first_name = item.foaf_name.split(' ', 1)[0]
            item.last_name = item.foaf_name.split(' ', 1)[1]
            item.save()

        for item in orm.DcContributor.objects.all():
            item.first_name = item.foaf_name.split(' ', 1)[0]
            item.last_name = item.foaf_name.split(' ', 1)[1]
            item.save()
    ...
    def backwards(self, orm):
        ...
        for item in orm.DcCreator.objects.all():
            item.foaf_name = "%s %s" % (item.first_name, item.last_name)
            item.save()
